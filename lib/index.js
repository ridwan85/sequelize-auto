  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if(!self.options.typescript){
        ...
        text[table] += 'import db from \'../utils/database/postgres\';\n';
        text[table] += `const ${_snakeToPascal(tableName)} = (sequelize, DataTypes) =>{\n`;
      ...
      _.each(fields, function(field, i){
          ...
          if( additional && additional.timestamps !== undefined && additional.timestamps){
            if((additional.createdAt && field === 'created_at' || additional.createdAt === field )
              ||(additional.updatedAt && field === 'updated_at' || additional.updatedAt === field )
              ||(additional.deletedAt && field === 'deleted_at' || additional.deletedAt === field )){
              return true
            }
          }
        // Find foreign key
      ...
      text[table] += spaces + spaces  + "createdAt: 'created_at',\n";

      text[table] += spaces + spaces  + "updatedAt: 'updated_at',\n";

      text[table] += spaces + spaces  + "deletedAt: 'deleted_at',\n";
      ...
      text[table] += ");\n};\n";
      text[table] += `export default ${_snakeToPascal(tableName)}(db, Sequelize);`;
      _callback(null);
    }, function(){
